...
def test_generate_digest_drops_invalid_key_story_refs(monkeypatch, tmp_path):
    from backend.config import settings
    monkeypatch.setattr(settings, "DB_PATH", str(tmp_path / "test.db"))
    from backend.db import init_db, get_conn
    init_db()
    ...
    canned = json.dumps({
        ...
        "themes": [],
        "key_stories": [
            {"ref": 2, "title": "WRONG TITLE", "why": "valid"},
            {"ref": 99, "title": "ghost", "why": "out of range"},
            {"ref": "x", "title": "bad", "why": "not a number"},
            {"title": "no ref", "why": "missing ref"},
        ],
        "context": "ctx",
        "outlook": "out",
    })
    ...
    assert stories[0]["article_id"] == 1
    assert stories[0]["title"] == "AI"
    assert stories[0]["why"] == "valid"
